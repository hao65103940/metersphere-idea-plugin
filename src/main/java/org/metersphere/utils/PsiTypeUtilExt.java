 ...
 * @Description:
 */
public class PsiTypeUtilExt {

    // 暂时存放类型泛型中真实信息类型
    public static final Map<String, PsiType[]> GENERIC_MAP = new HashMap<>();

    private static Logger logger = Logger.getInstance(PsiTypeUtilExt.class);
    ...
    public static void resolvePsiClassParameter(PsiClassType psiClassReferenceType) {
        ...
        if (psiClass == null) {
            PsiTypeUtilExt.handleSyntaxError(psiClassReferenceType.getCanonicalText());
        }
    ...
    public static boolean isPsiClassFromXxx(PsiClass psiClass, Project project, String qNameOfXxx) {
        ...
        PsiClass xxxClass = JavaPsiFacade.getInstance(project).findClass(qNameOfXxx, GlobalSearchScope.allScope(project));
